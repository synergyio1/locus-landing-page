...
> **Superseded as *tab names* on 2026-08-31.** The four roles above are still the loop's vocabulary, but none of them is a tab any more — see **App IA** below before naming any screen in copy.

## App IA (2026-08-31 — six flat tabs + titlebar chat)

Shipped in the product's Execution re-org (PRDs #725–#738, 2026-08-27/28; ground truth `pomodoro-preview/…/LocusUI/SidebarTab.swift`, `docs/exec-plans/active/execution-reorg-six-tabs.md`, `CONTEXT.md`). The sidebar is **six flat tabs, no families, sidebar order = shortcut order**:

| ⌘ | Tab | Subtitle (verbatim) | Job |
|---|---|---|---|
| 1 | **Home** | How it's going, across everything | Dashboard — the record, never the verdict. Runs nothing. No streaks, no percentages. |
| 2 | **Vision** | Where you're going | Your why, in your own words (five-question interview). Second pane: the **Portrait** — Locus's read of you — and the distance between the two. |
| 3 | **Path** | How you get there | Every Commitment — Projects (finish line) · Habits (repeat) · Targets (amount of time) — and the Tasks inside them. Panes: Tasks · Commitments. No calendar. |
| 4 | **Execution** | When, and what happened | One time axis, one divider: intended on the left, what happened on the right (inferred blocks, described never scored). Sessions run here. Day · Week lenses. "kept 4 of 6 blocks", never a percentage. |
| 5 | **Notes** | Capture and analyze | Capture; Analyze → next-step suggestions; research dive; auto-title. |
| 6 | **System** | Your autopilot | The **Protocols** Locus runs without being asked (Markdown files: triggers, tools, depth). A **Method** swaps the whole coaching style. *The page header still reads "Intelligence" in-app — copy says System.* |

**Chat is not a tab.** It lives in the **title bar behind the Locus mark (⌘J)**, opens as a panel beside whatever screen you're on (or expands to the whole window), **inherits the current screen's context** ("this one" = that project/block/note), reads everything and acts (propose-first / done-cards with Undo; **Ask · Standard · Auto**; **Fast · Deep**). **Voice is ⌥1** — press once and talk; on-device speech-to-text, the reply read back in a voice, no Locus server in either path. Placeholder in-app: *"…or press ⌥1 and just talk."*

**Retirement map** (old → where it lives now): Focus, Watch, Command, Sentinel → **Execution**; Tasks, Commitments, Projects, Habits → **Path**; Portrait, Memory → **Vision**; Routines, Intelligence, Protocols → **System**; Flywheel, Review → **chat**. Vocabulary: *Routine → Protocol* (PRD #730), *routine pack → Method*, *Rhythm → Habit* (2026-08-31), *off-session* retired (a **Block** is declared or inferred). The spine that connects the tabs — **Divergence**: three levels, two gaps (what the Vision implies · what you said you'd do · what you did); plan-vs-reality is computed on Execution, Vision-vs-plan is raised in prose on Path.
